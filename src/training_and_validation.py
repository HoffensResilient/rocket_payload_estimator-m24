...
def train_and_evaluate(X_train, X_test, y_train, y_test, model_name, model, feature_set_name, feature_set):
    ...
    fig.suptitle(f'{model_name} - Training Data: Predicted vs Actual Values ({feature_set_name})', fontsize=10)
    ...
    axes[1, 2].text(0.5, 0.5, textstr, transform=axes[1, 2].transAxes, fontsize=10,
    ...
    fig, axes = plt.subplots(2, 3, figsize=(10, 6))
    fig.suptitle(f'{model_name} - Test Data: Predicted vs Actual Values ({feature_set_name})', fontsize=10)
...
def validate_on_real_data(X_real, y_real_actual, has_transfer_real, trained_models, rocket_names):
    ...
    print(f"Total rockets: {len(has_transfer_real)}")
    print(f"Rockets with transfer stage: {has_transfer_real.sum()}")
    print(f"Rockets without transfer stage: {(~has_transfer_real).sum()}")
    ...
    # DEBUG: Print rocket names in each category
    print(f"Rockets with transfer stage: {rocket_names_with.tolist()}")
    print(f"Rockets without transfer stage: {rocket_names_without.tolist()}")
    ...
    if len(X_real_with_transfer) > 0 and models_with_transfer_available:
        ...
        print(f"\nSkipping validation for rockets with transfer stage.")
        print(f"Reason: {len(X_real_with_transfer)} rockets with transfer stage, models available: {models_with_transfer_available}")
    ...
    if len(X_real_without_transfer) > 0 and models_without_available:
        ...
        print(f"\nSkipping validation for rockets without transfer stage.")
        print(f"Reason: {len(X_real_without_transfer)} rockets without transfer stage, models available: {models_without_available}")
...
# Modify the validate_model_group function to use aligned features
def validate_model_group(X_real, y_real, rocket_names, trained_models, group_name, X_train_columns):
    """
    Validate a group of models on real data with aligned features
    """
    predictions_dict = {}
    metrics_dict = {}
...
def create_validation_comparison_plots(validation_results):
    """
    Create comparison plots for validation results (robust checks).
    """
    print(f"\n{'='*80}")
    ...
    for group_name, group_results in validation_results.items():
        ...
        fig.suptitle(f'Validation Results - {group_name} Rockets: Predicted vs Actual Payload', fontsize=12)
        ...
        for i, orbit in enumerate(orbit_names):
            ...
            if orbit in actual_values.columns:
                actual = actual_values[orbit].values.astype(float)
                ...
                if valid_mask.sum() > 0:
                    ...
                    # Plot predictions from each model (if present)
                    ...
                    for j, (model_name, pred) in enumerate(predictions.items()):
                        ...
                        # safety: if pred has fewer rows than actual, skip
                        if pred.shape[0] != len(actual):
                            print(f"  Warning: predictions for model {model_name} have shape {pred.shape}, expected {len(actual)} rows. Aligning by index.")
                            # attempt to align by length: if pred shorter, skip plotting
                            if pred.shape[0] < len(actual):
                                continue
                        # take only the indices where actual exists
                        pred_orbit = pred[valid_mask, i]
                        ax.scatter(actual_valid, pred_orbit, alpha=0.8, label=model_name, color=colors[j % len(colors)])
                        plotted_any = True

                    if not plotted_any:
                        ax.text(0.5, 0.5, 'No predictions available', transform=ax.transAxes, ha='center', va='center')
                    else:
                        ...
                        # Annotations (if small dataset)
                        if len(actual_valid) <= 12:
                            for idx_local, rocket_label in enumerate(rocket_names_valid):
                                x = actual_valid[idx_local]
                                # pick first model prediction for annotation coordinate (if exists)
                                y_for_annotation = None
                                for pred in predictions.values():
                                    if pred is None: 
                                        continue
                                    pred = np.asarray(pred)
                                    if pred.shape[0] == len(actual):
                                        y_for_annotation = pred[valid_mask, i][idx_local]
                                        break
                                if y_for_annotation is None or np.isnan(y_for_annotation):
                                    y_for_annotation = x  # fallback
                                ax.annotate(rocket_label, (x, y_for_annotation), xytext=(5, 5), textcoords='offset points', fontsize=7, alpha=0.8)

                else:
                    ax.text(0.5, 0.5, 'No valid actuals', transform=ax.transAxes, ha='center', va='center', fontsize=10)
                    ax.set_title(f'{orbit} Orbit - No Data')
            else:
                ax.text(0.5, 0.5, 'Orbit not present in actuals', transform=ax.transAxes, ha='center', va='center', fontsize=10)
...
def create_error_analysis_plots(validation_results):
    ...
    fig.suptitle('Model Performance Comparison on Real Data', fontsize=10)
